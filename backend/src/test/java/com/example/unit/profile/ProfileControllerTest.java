...
class ProfileControllerTest {
    ...
    void testUpdateProfile_Success() {
        when(profileService.updateProfile(1L, profile)).thenReturn(profile);
    ...
    void testUpdateProfile_Failure_NotFound() {
        when(profileService.updateProfile(9999L, profile)).thenReturn(null);
    ...
    void testDeleteProfile_Failure_NotFound() {
        doThrow(new IllegalArgumentException("Profile with id 9999 does not exist"))
                .when(profileService).deleteProfile(9999L);
    ...
    void testUpdateRating_Failure_NotFound() {
        ...
        assertEquals(404, response.getStatusCodeValue());
        verify(profileService, never()).updateRating(9999L, 4.8);
    }
}
